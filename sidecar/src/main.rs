...
use fuser::{
    ...
    RenameFlags, ReplyOpen, ReplyWrite, Request, TimeOrNow, WriteFlags,
};
...
fn ino_u64(ino: INodeNo) -> u64 { ino.0 }
fn fh_u64(fh: FileHandle) -> u64 { fh.0 }

impl Inner {
    fn new(client: tarantool::Client, pass_root: PathBuf) -> Self {
        if !pass_root.exists() { let _ = fs::create_dir_all(&pass_root); }
        let mut s = Self {
            tt: client,
            pass_root,
            inodes: HashMap::new(),
    ...
    fn list_plugins(&mut self) -> Vec<String> {
        let mut api = tarantool::LoomApi { client: &mut self.tt };
        api.list_plugins().unwrap_or_default()
    }

    fn list_files(&mut self, plugin: &str) -> Vec<tarantool::FileMeta> {
        let mut api = tarantool::LoomApi { client: &mut self.tt };
        api.list_files(plugin).unwrap_or_default()
    }

    fn get_file(&mut self, plugin: &str, filename: &str) -> Option<tarantool::FileEntry> {
        let mut api = tarantool::LoomApi { client: &mut self.tt };
        api.get_file(plugin, filename).ok().flatten()
    }

    fn upsert_file(&mut self, plugin: &str, filename: &str, raw: &[u8]) -> Result<()> {
        let mut api = tarantool::LoomApi { client: &mut self.tt };
        api.upsert_file(plugin, filename, raw)
    }

    fn delete_file(&mut self, plugin: &str, filename: &str) -> Result<()> {
        let mut api = tarantool::LoomApi { client: &mut self.tt };
        api.delete_file(plugin, filename)
    }

    fn list_dir_children(&mut self, plugin: &str, prefix: &str) -> Vec<(String, bool)> {
        let pws = if prefix.is_empty() { String::new() } else { format!("{}/", prefix) };
        let files = self.list_files(plugin);
        let mut seen: HashSet<String> = HashSet::new();
        let mut out: Vec<(String, bool)> = Vec::new();
        for f in &files {
            if !f.filename.starts_with(&pws) && !pws.is_empty() { continue; }
            let rest = &f.filename[pws.len()..];
            if rest.is_empty() { continue; }
            match rest.find('/') {
                None => { if seen.insert(format!("F:{}", rest)) { out.push((rest.to_string(), true)); } }
                Some(idx) => {
                    let dirname = &rest[..idx];
                    if seen.insert(format!("D:{}", dirname)) { out.push((dirname.to_string(), false)); }
                }
            }
        }
        for (gp, gd) in &self.ghost_dirs {
            if gp != plugin { continue; }
            let parent = match gd.rfind('/') { Some(i) => &gd[..i], None => "" };
            if parent == prefix {
                let child = match gd.rfind('/') { Some(i) => &gd[i + 1..], None => gd.as_str() };
                if seen.insert(format!("D:{}", child)) { out.push((child.to_string(), false)); }
            }
        }
        out
    }
    ...
    fn persist_fh(&mut self, fh: u64) -> Result<()> {
        ...
        let (pass_real, plugin, filename, bytes) = {
            ...
            (b.pass_real.clone(), b.plugin.clone(), b.filename.clone(), b.bytes.clone())
        };
        if let Some(real) = pass_real {
            if let Some(parent) = real.parent() { let _ = fs::create_dir_all(parent); }
            let mut f = fs::OpenOptions::new()
                .write(true).create(true).truncate(true).open(&real)?;
            f.write_all(&bytes)?;
            f.flush()?;
        } else {
            self.upsert_file(&plugin, &filename, &bytes)?;
            self.ghost_dirs.retain(|(p, d)|
                !(p == &plugin && filename.starts_with(&format!("{}/", d)))
            );
        }
        if let Some(b) = self.open_files.get_mut(&fh) { b.dirty = false; }
        Ok(())
    }
}
...
fn fs_path_for(plugin: &str, prefix: &str) -> String {
    if prefix.is_empty() { format!("/{}", plugin) } else { format!("/{}/{}", plugin, prefix) }
}
...
fn pass_attr(ino: u64, real: &Path) -> Option<FileAttr> {
    ...
    let ftype = if m.is_dir() { FileType::Directory } else { FileType::RegularFile };
    let perm = (m.permissions().mode() & 0o7777) as u16;
    let mtime = m.modified().unwrap_or(UNIX_EPOCH);
    Some(FileAttr {
        ino: INodeNo(ino), size: m.size(), blocks: m.blocks(),
        atime: mtime, mtime, ctime: mtime, crtime: mtime,
        kind: ftype, perm, nlink: m.nlink() as u32,
        uid: 1000, gid: 1000, rdev: 0, blksize: 4096, flags: 0,
    })
}
...
impl Filesystem for LoomFS {
    fn lookup(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEntry) {
        ...
        let name_str = match name.to_str() {
            Some(s) => s,
            None => { reply.error(fuser::Errno::from_i32(ENOENT)); return; }
        };
        ...
        match s.inodes.get(&parent).cloned() {
            Some(Entry::Root) => {
                ...
                if let Some(&ino) = s.paths.get(&path) {
                    match s.inodes.get(&ino).cloned() {
                        Some(Entry::Dir { .. }) => { reply.entry(&TTL, &dir_attr(ino), fuser::Generation(0)); return; }
                        Some(Entry::Pass { real, .. }) => {
                            if let Some(attr) = pass_attr(ino, &real) { reply.entry(&TTL, &attr, fuser::Generation(0)); return; }
                        }
                        _ => {}
                    }
                }
                ...
                if let Ok(meta) = fs::symlink_metadata(&real) {
                    ...
                    let ino = s.alloc_inode(&path, Entry::Pass { real: real.clone(), is_dir });
                    if let Some(attr) = pass_attr(ino, &real) { reply.entry(&TTL, &attr, fuser::Generation(0)); return; }
                }
                let plugins = s.list_plugins();
                if plugins.iter().any(|p| p == name_str) {
                    let ino = s.alloc_inode(&path, Entry::Dir {
            ...
            Some(Entry::Dir { plugin, prefix }) => {
                ...
                if let Some(entry) = s.get_file(&plugin, &child_path) {
                    ...
                    let ino = s.alloc_inode(&fp, Entry::File {
                        plugin: plugin.clone(),
                        filename: child_path,
                    });
                    reply.entry(&TTL, &file_attr(ino, entry.meta.size, entry.meta.mtime), fuser::Generation(0));
                    return;
                }
                ...
                if is_real_dir || is_ghost {
                    let fp = fs_path_for(&plugin, &child_path);
                    let ino = s.alloc_inode(&fp, Entry::Dir { plugin, prefix: child_path });
                    reply.entry(&TTL, &dir_attr(ino), fuser::Generation(0));
                } else {
                    reply.error(fuser::Errno::from_i32(ENOENT));
                }
            }
            Some(Entry::Pass { real: parent_real, is_dir: true }) => {
                ...
                if let Ok(_meta) = fs::symlink_metadata(&real) {
                    ...
                    let ino = s.alloc_inode(&key, Entry::Pass { real: real.clone(), is_dir });
                    if let Some(attr) = pass_attr(ino, &real) { reply.entry(&TTL, &attr, fuser::Generation(0)); return; }
                }
                reply.error(fuser::Errno::from_i32(ENOENT));
            }
            Some(Entry::Pass { is_dir: false, .. }) | Some(Entry::File { .. }) => reply.error(fuser::Errno::from_i32(ENOTDIR)),
        ...
        match s.inodes.get(&ino).cloned() {
            Some(Entry::File { plugin, filename }) => {
                if let Some(new_size) = size {
                    let mut bytes = s.get_file(&plugin, &filename).map(|e| e.raw).unwrap_or_default();
                    bytes.resize(new_size as usize, 0);
                    let _ = s.upsert_file(&plugin, &filename, &bytes);
                    for buf in s.open_files.values_mut() {
                        if buf.pass_real.is_none() && buf.plugin == plugin && buf.filename == filename {
                ...
                let sz = s.get_file(&plugin, &filename).map(|e| e.meta.size).unwrap_or(0);
                reply.attr(&TTL, &file_attr(ino, sz, now));
            }
            Some(Entry::Pass { real, is_dir: false }) => {
                if let Some(new_size) = size {
                ...
                if let Some(attr) = pass_attr(ino, &real) { reply.attr(&TTL, &attr); }
                else { reply.error(fuser::Errno::from_i32(ENOENT)); }
            }
            Some(Entry::Pass { is_dir: true, real }) => {
                if let Some(attr) = pass_attr(ino, &real) { reply.attr(&TTL, &attr); }
                else { reply.attr(&TTL, &dir_attr(ino)); }
            }
    ...
    fn readdir(
        &self, _req: &Request, ino: INodeNo, _fh: FileHandle, offset: u64, mut reply: ReplyDirectory,
    ) {
        let ino = ino_u64(ino);
        let mut s = self.state.lock().unwrap();
        ...
        match s.inodes.get(&ino).cloned() {
            Some(Entry::Root) => {
                for plugin in s.list_plugins() {
                    ...
                    let i = s.alloc_inode(&path, Entry::Dir {
                        plugin: plugin.clone(),
                        prefix: String::new(),
                    });
            ...
            Some(Entry::Dir { plugin, prefix }) => {
                for (name, is_file) in s.list_dir_children(&plugin, &prefix) {
                    ...
                    if is_file {
                        let i = s.alloc_inode(&fp, Entry::File {
                            plugin: plugin.clone(),
                            filename: child,
                        });
                        entries.push((i, FileType::RegularFile, name));
                    } else {
                        let i = s.alloc_inode(&fp, Entry::Dir {
                            plugin: plugin.clone(),
                            prefix: child,
                        });
            ...
            _ => { reply.error(fuser::Errno::from_i32(ENOTDIR)); return; }
        }
        for (i, e) in entries.into_iter().enumerate().skip(offset as usize) {
            if reply.add(INodeNo(e.0), (i + 1) as u64, e.1, &e.2) { break; }
        }
        reply.ok();
    }
        ...
        match s.inodes.get(&ino).cloned() {
            Some(Entry::File { plugin, filename }) => {
                let bytes = if truncate {
                    ...
                    s.get_file(&plugin, &filename).map(|e| e.raw).unwrap_or_default()
                };
                let fh = s.next_fh; s.next_fh += 1;
                s.open_files.insert(fh, WriteBuf {
                    bytes, plugin, filename,
                    pass_real: None, dirty: truncate,
                });
                reply.opened(FileHandle(fh), fuser::FopenFlags::empty());
            }
            Some(Entry::Pass { real, is_dir: false }) => {
                let bytes = if truncate { Vec::new() } else { fs::read(&real).unwrap_or_default() };
                let fh = s.next_fh; s.next_fh += 1;
                s.open_files.insert(fh, WriteBuf {
                    bytes, plugin: String::new(),
                    filename: real.display().to_string(),
                    pass_real: Some(real), dirty: truncate,
                });
                reply.opened(FileHandle(fh), fuser::FopenFlags::empty());
            }
    ...
    fn read(
        &self, _req: &Request, _ino: INodeNo, fh: FileHandle, offset: u64, size: u32,
        _flags: OpenFlags, _lock: Option<LockOwner>, reply: ReplyData,
    ) {
        let fh = fh_u64(fh);
        let s = self.state.lock().unwrap();
        let buf = match s.open_files.get(&fh) { Some(b) => b, None => { reply.error(fuser::Errno::from_i32(EIO)); return; } };
        let off = offset as usize;
        if off >= buf.bytes.len() { reply.data(&[]); return; }
        let end = (off + size as usize).min(buf.bytes.len());
        reply.data(&buf.bytes[off..end]);
    }

    fn write(
        &self, _req: &Request, _ino: INodeNo, fh: FileHandle, offset: u64, data: &[u8],
        _wf: WriteFlags, _flags: OpenFlags, _lo: Option<LockOwner>, reply: ReplyWrite,
    ) {
        let fh = fh_u64(fh);
        let mut s = self.state.lock().unwrap();
        let buf = match s.open_files.get_mut(&fh) { Some(b) => b, None => { reply.error(fuser::Errno::from_i32(EIO)); return; } };
        let off = offset as usize;
        let end = off + data.len();
        if end > buf.bytes.len() { buf.bytes.resize(end, 0); }
    ...
    fn flush(&self, _req: &Request, _ino: INodeNo, fh: FileHandle, _lock: LockOwner, reply: ReplyEmpty) {
        let fh = fh_u64(fh);
        let mut s = self.state.lock().unwrap();
        if s.persist_fh(fh).is_err() { reply.error(fuser::Errno::from_i32(EIO)); return; }
        reply.ok();
    }

    fn release(
        &self, _req: &Request, _ino: INodeNo, fh: FileHandle, _flags: OpenFlags,
        _lo: Option<LockOwner>, _flush: bool, reply: ReplyEmpty,
    ) {
        let fh = fh_u64(fh);
        let mut s = self.state.lock().unwrap();
    ...
    fn create(
        &self, _req: &Request, parent: INodeNo, name: &OsStr, _mode: u32, _umask: u32,
        _flags: i32, reply: ReplyCreate,
    ) {
        let parent = ino_u64(parent);
        let name_str = match name.to_str() {
            Some(s) => s.to_string(),
            None => { reply.error(fuser::Errno::from_i32(EIO)); return; }
        };
        let now = SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0);
        let mut s = self.state.lock().unwrap();
        match s.inodes.get(&parent).cloned() {
            Some(Entry::Root) => {
                let real = s.pass_root.join(&name_str);
                if let Some(p) = real.parent() { let _ = fs::create_dir_all(p); }
                if let Err(e) = fs::File::create(&real) { error!(error = %e, "pass create"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = format!("/{}", name_str);
                let ino = s.alloc_inode(&key, Entry::Pass { real: real.clone(), is_dir: false });
                let attr = pass_attr(ino, &real).unwrap_or_else(|| file_attr(ino, 0, now));
                let fh = s.next_fh; s.next_fh += 1;
                s.open_files.insert(fh, WriteBuf {
                    bytes: Vec::new(), plugin: String::new(),
                    filename: real.display().to_string(),
                    pass_real: Some(real), dirty: false,
                });
                reply.created(&TTL, &attr, fuser::Generation(0), FileHandle(fh), fuser::FopenFlags::empty());
            }
            Some(Entry::Dir { plugin, prefix }) => {
                let filename = join_path(&prefix, &name_str);
                if let Err(e) = s.upsert_file(&plugin, &filename, &[]) {
                    error!(error = %e, "create upsert"); reply.error(fuser::Errno::from_i32(EIO)); return;
                }
                ...
                let fh = s.next_fh; s.next_fh += 1;
                s.open_files.insert(fh, WriteBuf {
                    bytes: Vec::new(), plugin, filename,
                    pass_real: None, dirty: false,
                });
                reply.created(&TTL, &attr, fuser::Generation(0), FileHandle(fh), fuser::FopenFlags::empty());
            }
            Some(Entry::Pass { real: parent_real, is_dir: true }) => {
                let real = parent_real.join(&name_str);
                if let Some(p) = real.parent() { let _ = fs::create_dir_all(p); }
                if let Err(e) = fs::File::create(&real) { error!(error = %e, "pass-sub create"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = pass_fs_path(&real);
                let ino = s.alloc_inode(&key, Entry::Pass { real: real.clone(), is_dir: false });
                let attr = pass_attr(ino, &real).unwrap_or_else(|| file_attr(ino, 0, now));
                let fh = s.next_fh; s.next_fh += 1;
    ...
    fn unlink(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEmpty) {
        ...
        match s.inodes.get(&parent).cloned() {
            Some(Entry::Root) => {
                let real = s.pass_root.join(name_str);
                if let Err(e) = fs::remove_file(&real) { error!(error = %e, "pass unlink"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = format!("/{}", name_str);
                if let Some(ino) = s.paths.remove(&key) { s.inodes.remove(&ino); }
                reply.ok();
            }
            Some(Entry::Dir { plugin, prefix }) => {
                let filename = join_path(&prefix, name_str);
                if let Err(e) = s.delete_file(&plugin, &filename) { error!(error = %e, "delete"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let fp = fs_path_for(&plugin, &filename);
                if let Some(ino) = s.paths.remove(&fp) { s.inodes.remove(&ino); }
                reply.ok();
            }
            Some(Entry::Pass { real: parent_real, is_dir: true }) => {
                let real = parent_real.join(name_str);
                if let Err(e) = fs::remove_file(&real) { error!(error = %e, "pass-sub unlink"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = pass_fs_path(&real);
                if let Some(ino) = s.paths.remove(&key) { s.inodes.remove(&ino); }
                reply.ok();
            }
    ...
    fn mkdir(
        ...
        match s.inodes.get(&parent).cloned() {
            Some(Entry::Dir { plugin, prefix }) => {
                ...
                let ino = s.alloc_inode(&fp, Entry::Dir { plugin, prefix: dirpath });
                reply.entry(&TTL, &dir_attr(ino), fuser::Generation(0));
            }
            Some(Entry::Root) => {
                if is_passthrough_root_name(&name_str) {
                    let real = s.pass_root.join(&name_str);
                    if let Err(e) = fs::create_dir_all(&real) { error!(error = %e, "pass mkdir"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                    let key = format!("/{}", name_str);
                    let ino = s.alloc_inode(&key, Entry::Pass { real: real.clone(), is_dir: true });
            ...
            Some(Entry::Pass { real: parent_real, is_dir: true }) => {
                let real = parent_real.join(&name_str);
                if let Err(e) = fs::create_dir_all(&real) { error!(error = %e, "pass-sub mkdir"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = pass_fs_path(&real);
                let ino = s.alloc_inode(&key, Entry::Pass { real: real.clone(), is_dir: true });
                let attr = pass_attr(ino, &real).unwrap_or_else(|| dir_attr(ino));
                reply.entry(&TTL, &attr, fuser::Generation(0));
    ...
    fn rmdir(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEmpty) {
        ...
        match s.inodes.get(&parent).cloned() {
            Some(Entry::Dir { plugin, prefix }) => {
                ...
                let files = s.list_files(&plugin);
                if files.iter().any(|f| f.filename.starts_with(&pref)) { reply.error(fuser::Errno::from_i32(libc::ENOTEMPTY)); return; }
                if s.ghost_dirs.iter().any(|(p, d)| p == &plugin && d.starts_with(&pref)) { reply.error(fuser::Errno::from_i32(libc::ENOTEMPTY)); return; }
            ...
            Some(Entry::Root) => {
                let real = s.pass_root.join(name_str);
                if let Err(e) = fs::remove_dir(&real) { error!(error = %e, "pass rmdir"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = format!("/{}", name_str);
                if let Some(ino) = s.paths.remove(&key) { s.inodes.remove(&ino); }
                reply.ok();
            }
            Some(Entry::Pass { real: parent_real, is_dir: true }) => {
                let real = parent_real.join(name_str);
                if let Err(e) = fs::remove_dir(&real) { error!(error = %e, "pass-sub rmdir"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let key = pass_fs_path(&real);
                if let Some(ino) = s.paths.remove(&key) { s.inodes.remove(&ino); }
                reply.ok();
            }
    ...
    fn rename(
        ...
        let n1 = match name.to_str() { Some(s) => s, None => { reply.error(fuser::Errno::from_i32(ENOENT)); return; } };
        let n2 = match newname.to_str() { Some(s) => s.to_string(), None => { reply.error(fuser::Errno::from_i32(EIO)); return; } };
        let mut s = self.state.lock().unwrap();
        ...
        if let (Some(from), Some(to)) = (from_pass, to_pass) {
            if from.exists() {
                if let Err(e) = fs::rename(&from, &to) { error!(error = %e, "pass rename"); reply.error(fuser::Errno::from_i32(EIO)); return; }
                let from_key = match &p1 {
                    Some(Entry::Root) => format!("/{}", n1),
                    _ => pass_fs_path(&from),
                };
                if let Some(ino) = s.paths.remove(&from_key) { s.inodes.remove(&ino); }
                reply.ok();
                return;
            }
        }

        if let (Some(Entry::Dir { plugin: pa, prefix: pra }), Some(Entry::Dir { plugin: pb, prefix: prb })) = (p1, p2) {
            ...
            if let Err(e) = s.upsert_file(&pb, &new_full, &entry.raw) { error!(error = %e, "rename upsert"); reply.error(fuser::Errno::from_i32(EIO)); return; }
            if !(pa == pb && old_full == new_full) { let _ = s.delete_file(&pa, &old_full); }
            let old_fs = fs_path_for(&pa, &old_full);
            if let Some(ino) = s.paths.remove(&old_fs) { s.inodes.remove(&ino); }
            reply.ok();
            return;
        }
...
fn main() -> Result<()> {
    ...
    let pass_dir = PathBuf::from(std::env::var("PASSTHROUGH_DIR").unwrap_or_else(|_| "/passthrough".into()));
    let user = std::env::var("LOOM_USER").ok().filter(|s| !s.is_empty());
    let pass = std::env::var("LOOM_PASSWORD").ok().filter(|s| !s.is_empty());
    ...
    let client = tarantool::Client::connect_with_retry_auth(&tarantool_url, auth, 30, 2000)?;
    info!("tarantool connected");

    let fs_impl = LoomFS { state: Mutex::new(Inner::new(client, pass_dir.clone())) };
    ...
    config.mount_options.push(MountOption::FSName("loom".into()));
